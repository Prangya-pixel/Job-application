...
export default function JobForm(){
  ...
  return <section className="auth admin-form"><form className="form-card wide" onSubmit={submit}>
    ...
    <label>Job type<select value={f.jobType} onChange={e=>setF({...f,jobType:e.target.value})}>{['Full-Time','Part-Time','Internship','Contract','Remote'].map(x=><option key={x}>{x}</option>)}</select></label>
    <label>Experience level<select value={f.experienceLevel} onChange={e=>setF({...f,experienceLevel:e.target.value})}>{['Entry','Mid','Senior','Lead'].map(x=><option key={x}>{x}</option>)}</select></label>
    <label>Description<textarea required value={f.description} onChange={e=>setF({...f,description:e.target.value})}/></label>
